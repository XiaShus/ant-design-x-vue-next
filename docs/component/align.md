...
| 表达 | Sender | ✅ | ✅（含导出 `FooterRender` / `SubmitType` / `SenderHeaderProps` / `SenderComponents` / `ActionsComponents`；`suffix`/`actions`、Switch（含 `nativeElement` ref）、词槽 contentEditable、`insert`+cursor/`replaceCharacters`、`classNames/styles` root/content、`inputElement`、locale） | [文档](/component/sender) |
