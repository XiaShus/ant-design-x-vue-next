...
| --- | --- | --- | --- | --- |
| 通用 | Bubble | ✅ | ✅（含 System / Divider / `contentRender`+`InfoType` / `extra` / avatar BubbleSlot / streaming / typing 函数与 `step` 区间） | [文档](/component/bubble) |
| 通用 | Conversations | ✅ | ✅（含 `creation` / `Conversations.Creation`、`shortcutKeys`、`groupable.collapsible` + 折叠动效、`onActiveChange(key, item)`、divider items；移动端菜单常显） | [文档](/component/conversations) |
...
| 表达 | Sender | ✅ | ✅（含 `suffix`/`actions`、Switch、词槽 contentEditable、`insert(string|slots)`+cursor/`replaceCharacters`、locale） | [文档](/component/sender) |
...
| 反馈 | Actions | ✅ | ✅（含 Copy / Feedback / Item / Audio、`fadeIn`/`fadeInLeft`、`dropdownProps`、`outlined`/`filled`） | [文档](/component/actions) |
