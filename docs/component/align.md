...
| 确认 | ThoughtChain | ✅ | ✅（含导出 `ThoughtChainSemanticType` / `TooltipConfig` / `ThoughtChainItemType`；`ThoughtChain.Item`（含 `nativeElement` ref）；链级 `line` / expand；items `status`：`loading`/`abort`；`blink`·`collapsible`·`destroyOnHidden`；`classNames/styles` root/item/itemHeader/itemIcon/itemContent/itemFooter；链级 `nativeElement` ref） | [文档](/component/thought-chain) |
