...
| 包 | React | Vue Next |
| --- | --- | --- |
| `@ant-design/x-sdk`（含 XRequest / XStream / Chat 能力） | ✅ | 内置：timeout/retry/abort + `setXRequestGlobalOptions` + `AbstractChatProvider` / OpenAI / DeepSeek + `useXConversations` + async `defaultMessages` / `queueRequest` / `extraInfo` + `XMCPClient`（未拆独立包；不含完整 AbstractXRequestClass 驱动） |
