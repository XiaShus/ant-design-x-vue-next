...
| 通用 | Conversations | ✅ | ✅（含导出 `CreationAlign` / `GroupableProps` / `GroupCollapsible` / `GroupTitleRender` / `GroupSorter` / `GroupLabel`；`creation` / `Conversations.Creation`、`shortcutKeys`、`groupable.collapsible` + 折叠动效、`onActiveChange(key, item)`、divider、`classNames/styles` root/group；移动端菜单常显；`nativeElement` ref） | [文档](/component/conversations) |
