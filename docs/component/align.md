...
| 包 | React | Vue Next |
| --- | --- | --- |
| `@ant-design/x-sdk`（含 XRequest / XStream / Chat 能力） | ✅ | 内置增强：timeout/retry/abort + OpenAI/DeepSeek Provider + conversationKey + `XMCPClient`；完整 AbstractChatProvider 栈继续对齐 |
...
7. ~~SDK 稳定性 + OpenAI/DeepSeek Provider + XMCPClient~~ ✅（继续对齐 AbstractChatProvider 完整栈）
