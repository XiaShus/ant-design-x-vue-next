...
export const fadeOut = new Keyframes('antXFadeOut', {
  '0%': {
    opacity: 1,
  },
  '100%': {
...
type FadeToken = FullToken<'Prompts'> & { antCls: string; componentCls: string };

export const initFadeLeftMotion = (token: FadeToken, sameLevel = false): CSSInterpolation => {
  const { antCls } = token;
...
export const initFadeMotion = (token: FadeToken, sameLevel = false): CSSInterpolation => {
  const { antCls } = token;
