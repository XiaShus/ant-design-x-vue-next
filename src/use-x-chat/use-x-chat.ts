...
>(config: XChatConfig<AgentMessage, ParsedMessage, Input, Output>) {
  ...
  const innerOnRequest = (
    ...
    const useProviderRequest = !!(provider?.hasRequest && provider.request);
    if (!useProviderRequest && !agent) {
      throw new Error(
    ...
    if (requestParams && typeof requestParams === 'object' && 'message' in requestParams) {
      ...
      localMessages = toArray(local) as AgentMessage[];
      message = (localMessages[localMessages.length - 1] ?? requestParams) as AgentMessage;
    ...
    if (reload) {
      ...
      persistAndSetMessages((ori) => {
        let nextMessages = [
          ...ori,
          ...localMessages.map((m) => createMessage(m, 'local', extraInfo)),
        ];
        if (requestPlaceholder) {
    ...
    const handleRequestError = async (error: Error) => {
      ...
      if (isAbortLikeError(error)) {
        persistAndSetMessages((ori) =>
          ori
            ...
            .map((info) =>
              uid != null && info.id === uid ? { ...info, status: 'abort' as const } : info,
            ),
        );
      ...
      if (requestFallback) {
        ...
        persistAndSetMessages((ori) =>
          ori.map((info) => (info.id === uid ? { ...info, status: 'error' as const } : info)),
        );
      } else {
        persistAndSetMessages((ori) => ori.filter((info) => info.id !== lid && info.id !== uid));
      }
      abortControllerRef.value = null;
    };

    // Provider-owned manual XRequest path (aligned with @ant-design/x-sdk)
    if (useProviderRequest && provider?.request) {
      provider.injectRequest({
        onUpdate: (chunk, headers) => updateMessage('updating', chunk, [], headers),
        ...
        onError: (error) => {
          void handleRequestError(error instanceof Error ? error : new Error(String(error)));
        },
      });
      const input = provider.transformParams(
        (requestParams && typeof requestParams === 'object'
          ? requestParams
          : { message }) as Partial<Input>,
        getRequestMessages(),
        provider.request.options,
      );
      provider.request.run(input);
      return;
    }
  ...
  const onReload = (
    ...
    if (!agent && !(provider?.hasRequest && provider.request))
      throw new Error(
